...
//static int timerVal = 100; // f = 100 HZ, T = 1 / 100 = 10ms, 100 * 10ms = 1SEC
static int ledVal = 0;
struct timer_list timerLed;

keyled_data kl_data = {0};
//static unsigned long temp_timer_val = 0;
...
//    printk("ledVal : %#04x\n", (unsigned int) (ledVal));
	...
	ledVal = ~ledVal & 0xff;
	gpioLedSet(ledVal);
    mod_timer(t, get_jiffies_64() + kl_data.timer_val);
/*	if(kl_data.timer_val)
	{
		ledVal = ~ledVal & 0xff;
		gpioLedSet(ledVal);
	}*/
...
//	gpioLedSet(kbuff); // DO NOT USE IN THIS CODE!
    return count;
}
...
#endif
    if( _IOC_TYPE( cmd ) != IOCTLTIMER_MAGIC ) return -EINVAL;
    ...
    switch( cmd )
    {
        case TIMER_START :
			if(!timer_pending(&timerLed))
				kerneltimer_registertimer(kl_data.timer_val);
//			kl_data.timer_val = temp_timer_val;
            break;
        case TIMER_STOP :
			if(timer_pending(&timerLed))
				del_timer(&timerLed);
//			temp_timer_val = kl_data.timer_val;
//			kl_data.timer_val = 0;
            break;
        case TIMER_VALUE :
            err = copy_from_user((void *)&kl_data,(void *)arg,size);
//			temp_timer_val = kl_data.timer_val;
            break;
        default:
            err =-E2BIG;
